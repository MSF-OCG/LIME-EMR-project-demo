...
install_LIME() {
    ...
    cd $INSTALLATION_DIR
    if ! ANSIBLE_LOG_PATH=$LOG_DIR/lime_setup_stderr_${current_timestamp}.log ANSIBLE_DISPLAY_OK_HOSTS=no ANSIBLE_DISPLAY_SKIPPED_HOSTS=no ansible-playbook -i inventories/"$INVENTORY".ini playbook.yaml; then
